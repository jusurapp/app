...
async fn transcribe_inner(
    ...
    if cache_path.exists() {
        ...
        let segments: Vec<serde_json::Value> =
            serde_json::from_str(&cached).map_err(|e| format!("Failed to parse cache: {}", e))?;
        return Ok(segments);
    }
    ...
    if !ytdlp.status.success() {
        crate::log::log!(
            "[transcribe] yt-dlp stderr: {}",
            String::from_utf8_lossy(&ytdlp.stderr)
        );
        return Err(format!(
            "yt-dlp failed: {}",
            String::from_utf8_lossy(&ytdlp.stderr)
    ...
    let downloaded_audio = std::fs::read_dir(tmp_path)
        ...
        .find(|p| {
            p.file_name()
                .map_or(false, |n| n.to_string_lossy().starts_with("audio."))
        })
    ...
    crate::log::log!(
        "[transcribe] Converted to 16kHz WAV: {} bytes",
        std::fs::metadata(&audio16_path)
            .map(|m| m.len())
            .unwrap_or(0)
    );
    ...
    crate::log::log!(
        "[transcribe] Loading WhisperEngine with model: {} ({} bytes)",
        model_path.display(),
        model_size
    );
    ...
    let segments = tokio::task::spawn_blocking(move || {
        ...
        {
            let app_cb = app_for_progress.clone();
            let vid_cb = video_id_for_progress.clone();
            params.set_progress_callback_safe(move |progress: i32| {
                app_cb
                    .emit(
                        "translation-status",
                        TranslationStatus {
                            video_id: vid_cb.clone(),
    ...
    crate::log::log!(
        "[transcribe] Got {} segments. Arabic text:",
        raw_segments.len()
    );
    ...
    tauri::async_runtime::spawn(async move {
        ...
        match transcribe_inner(&app, &url, &video_id).await {
            Ok(segments) => {
                ...
                if let Ok(json) = serde_json::to_string(&metadata) {
                    let _ =
                        std::fs::write(cache_dir.join(format!("{}.meta.json", &video_id)), json);
                }
                app.emit("translation-completed", metadata).ok();
            }

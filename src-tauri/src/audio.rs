...
pub fn convert_to_wav_16k(input: &Path, output: &Path) -> Result<(), String> {
    ...
    // Open input
    let mut ictx =
        ffmpeg::format::input(input).map_err(|e| format!("Failed to open audio input: {e}"))?;
    ...
    // Open output (WAV)
    let mut octx =
        ffmpeg::format::output(output).map_err(|e| format!("Failed to create WAV output: {e}"))?;

    let codec =
        ffmpeg::encoder::find(ffmpeg::codec::Id::PCM_S16LE).ok_or("PCM_S16LE encoder not found")?;
